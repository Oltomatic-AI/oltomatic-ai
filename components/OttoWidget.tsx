...
export default function OttoWidget() {
  ...
  return (
    <>
      {expanded && (
        <div style={{ position:"fixed", bottom:"88px", right:"24px", width:"280px", background:"#0D0D1A", border:"1px solid rgba(21,96,168,0.3)", borderRadius:"16px", boxShadow:"0 24px 64px rgba(0,0,0,0.5)", zIndex:9999 }}>
          <div style={{ padding:"16px 20px", borderBottom:"1px solid #1E1E32", display:"flex", alignItems:"center", gap:"12px" }}>
            <div style={{ width:"36px", height:"36px", borderRadius:"8px", background:"#1560A8", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:"bold", color:"white", flexShrink:0 }}>O</div>
            <div>
            ...
            {isActive && <span style={{ marginLeft:"auto", color:"#4ade80", fontSize:"12px" }}>● Live</span>}
            {isConnecting && <span style={{ marginLeft:"auto", color:"#9999B0", fontSize:"12px" }}>Connecting...</span>}
          </div>
          <div style={{ padding:"20px" }}>
            {!isBusy ? (
              <>
                <p style={{ color:"#9999B0", fontSize:"14px", lineHeight:1.6, marginBottom:"16px" }}>Talk to OTTO — our AI agent. Ask anything about what we do or what we can build for you.</p>
                ...
                <p style={{ color:"#55556A", fontSize:"11px", textAlign:"center", marginTop:"10px" }}>Microphone required</p>
              </>
            ) : (
              <>
                <p style={{ color: isActive ? "#EEEEF5" : "#9999B0", fontSize:"14px", textAlign:"center", marginBottom:"16px" }}>
                  {isActive ? "OTTO is listening..." : "Connecting to OTTO..."}
                </p>
      ...
      <button
        onClick={() => isBusy ? endCall() : setExpanded(prev => !prev)}
        ...
        <div style={{ textAlign:"left" }}>
          ...
          <p style={{ color:"rgba(255,255,255,0.55)", fontSize:"11px", margin:0 }}>AI · Live</p>
        </div>
      </button>
    </>
  );
