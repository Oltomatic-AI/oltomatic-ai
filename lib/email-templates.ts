/**
 * Branded email templates — match the Oltomatic dark-mode aesthetic.
 * Inline CSS only (most email clients ignore <style> blocks).
 *
 * Note: we use brand-adjacent styling but keep the design light-mode
 * because many clients (Outlook, Gmail web) render dark emails poorly.
 * A white/ivory background with blue accents reads reliably everywhere.
 */
...
<html><body style="margin:0;padding:24px;background:${CANVAS};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:${TEXT_INK};">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid ${BORDER};border-radius:12px;overflow:hidden;">
    ...
    <div style="padding:16px 28px;border-top:1px solid ${BORDER};font-size:11px;color:${TEXT_MUTED};">
      ...
      <a href="https://oltomatic.ai" style="color:${BRAND_BLUE};text-decoration:none;">oltomatic.ai</a>
    </div>
  </div>
</body></html>`;
}
...
export function contactInternalHtml(p: ContactPayload): string {
  return shell(`
    ...
    <p style="margin:0 0 18px 0;color:${TEXT_MUTED};font-size:13px;">From ${p.site}</p>
    ...
    <p style="margin:22px 0 0 0;font-size:12px;color:${TEXT_MUTED};">Reply directly to this email to respond to the prospect.</p>
  `);
}
...
export function contactProspectHtml(p: ContactPayload): string {
  ...
  return shell(`
    <p style="margin:0 0 14px 0;font-size:18px;font-weight:600;">Hi ${escapeHtml(firstName)},</p>
    ...
    <p style="margin:0 0 14px 0;font-size:15px;line-height:1.6;">
      ...
      <a href="https://oltomatic.ai/contact" style="color:${BRAND_BLUE};">oltomatic.ai/contact</a>
    </p>
